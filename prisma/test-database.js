...
async function testDatabaseSchema() {
  ...
  try {
    ...
    if (projectWithData) {
      ...
      const criticalIssues = projectWithData.siteAudits.reduce((acc, audit) => 
        acc + audit.issues.length, 0
      )
...
async function main() {
  try {
    ...
    console.log(`   - Performance: ${performanceResults.performance === 'good' ? '✅ GOOD' : '⚠️  NEEDS IMPROVEMENT'}`)
