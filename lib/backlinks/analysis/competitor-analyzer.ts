...
export class CompetitorAnalyzer {
  ...
  analyzeCompetitor(
    ...
    const gapBacklinks = competitorBacklinks.filter(
      (cb) => !yourBacklinks.some((yb) => yb.sourceDomain === cb.sourceDomain)
    );

    // Your unique backlinks
    const yourUniqueBacklinks = yourBacklinks.filter(
      (yb) => !competitorBacklinks.some((cb) => cb.sourceDomain === yb.sourceDomain)
    );
  ...
  analyzeGaps(
    ...
    const opportunities = gapBacklinks.map((link) => this.scoreOpportunity(link, priorityKeywords));
  ...
  analyzeStrategy(backlinks: BacklinkData[]): {
    ...
    const brandedAnchors = backlinks.filter(
      (b) => b.anchorText && this.isBrandedAnchor(b.anchorText, "")
    ).length;
    ...
    const topSource = Object.entries(linkSources).sort((a, b) => b[1] - a[1])[0];
  ...
  compareMultipleCompetitors(
    ...
    const avgCompetitorLinks =
      competitors.reduce((sum, c) => sum + c.backlinks.length, 0) / competitors.length;
    if (yourBacklinks.length < avgCompetitorLinks * 0.5) {
      ...
      recommendations.push(
        `📊 Target: Reach at least ${Math.round(avgCompetitorLinks * 0.7)} backlinks`
      );
    }

    if (commonOpportunities.length > 50) {
      recommendations.push(
        `🎯 HIGH PRIORITY: ${commonOpportunities.length} opportunities where multiple competitors have links`
      );
    ...
    if (uniqueAdvantages.length > 20) {
      recommendations.push(
        `✅ You have ${uniqueAdvantages.length} unique quality backlinks competitors don't have`
      );
  ...
  private calculateAverageDR(backlinks: BacklinkData[]): number {
    const validDRs = backlinks.map((b) => b.domainRating || 0).filter((dr) => dr > 0);
  ...
  private generateCompetitorRecommendations(
    ...
    if (linkDifference > 100) {
      ...
      recommendations.push(
        `✅ You have ${Math.abs(linkDifference)} more backlinks than competitor`
      );
    }
    ...
    if (compAvgDR > yourAvgDR + 10) {
      recommendations.push(
        `⚠️ Competitor has higher quality links (Avg DR ${compAvgDR} vs ${yourAvgDR})`
      );
    ...
    if (gaps.length > 50) {
      ...
      recommendations.push(
        `🎯 ${gaps.length} link opportunities identified (${highQualityGaps} high-quality)`
      );
