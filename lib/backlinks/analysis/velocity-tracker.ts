...
export class LinkVelocityTracker {
  ...
  private detectSpikes(dailyData: VelocityDataPoint[]): VelocitySpikeDetection {
    ...
    const hasSuspiciousSpikes = spikes.some(s => s.severity === 'high' || s.severity === 'critical')
  ...
  private calculateTrend(weeklyData: VelocityDataPoint[]): VelocityTrend {
    ...
    const avgWeeklyGrowth = recentWeeks.reduce((sum, w) => sum + w.newLinks, 0) / recentWeeks.length
    
    // Calculate variance
    const variance = recentWeeks.reduce((sum, w) => {
  ...
  private generateRecommendations(
    ...
    if (spikeDetection.hasSuspiciousSpikes) {
      ...
      if (criticalSpikes.length > 0) {
        recommendations.push('🚨 CRITICAL: Detected massive link spikes - possible negative SEO attack or artificial link building')
        recommendations.push('⚡ ACTION: Review recent backlinks and use disavow file for suspicious links')
      } else if (highSpikes.length > 0) {
        recommendations.push('⚠️ WARNING: Detected significant link spikes - investigate source')
        recommendations.push('🔍 Review: Check if these are from legitimate sources or link schemes')
      }
    ...
    switch (trend.type) {
      case 'rapid':
        ...
        recommendations.push('💡 Recommendation: Slow down link acquisition to appear more natural')
        break
      ...
      case 'declining':
        ...
        recommendations.push('⚡ ACTION: Check for removed content, broken links, or domain issues')
  ...
  compareVelocity(
    ...
    if (yourGrowth < compGrowth * 0.5) {
      comparison.push(`🔴 You're acquiring links ${((compGrowth / yourGrowth) - 1) * 100}% slower than competitor`)
      comparison.push('💡 Increase link building efforts to catch up')
    } else if (yourGrowth > compGrowth * 2) {
      comparison.push(`✅ You're acquiring links ${((yourGrowth / compGrowth) - 1) * 100}% faster than competitor`)
