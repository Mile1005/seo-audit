...
export class AnchorTextAnalyzer {
  ...
  private calculateHealthScore(percentages: Record<string, number>): number {
    ...
    if (branded < 30)
      score -= 20; // Too few branded
    else if (branded > 70)
      score -= 10; // Too many branded
    else if (branded >= 40 && branded <= 60) score += 10; // Perfect range

    // Exact match penalties
    if (exact > 30)
      score -= 30; // Dangerous over-optimization
    else if (exact > 20)
      score -= 15; // High risk
    ...
    // Variety bonus
    const typesUsed = [branded, exact, partial, generic, naked].filter((pct) => pct > 5).length;
  ...
  compareProfiles(
    ...
    if (Math.abs(yourBranded - compBranded) > 15) {
      if (yourBranded < compBranded) {
        differences.push(
          `🔴 You have ${(compBranded - yourBranded).toFixed(1)}% fewer branded anchors than competitor`
        );
      } else {
        differences.push(
          `✅ You have ${(yourBranded - compBranded).toFixed(1)}% more branded anchors than competitor`
        );
      }
    }
    ...
    if (compExact > 25 && yourExact < compExact) {
      differences.push(
        "⚠️ Competitor using risky exact match strategy - opportunity to rank safer"
      );
    ...
    if (yours.healthScore > competitor.healthScore + 10) {
      differences.push(
        `✅ Your anchor profile is healthier (${yours.healthScore} vs ${competitor.healthScore})`
      );
    } else if (competitor.healthScore > yours.healthScore + 10) {
      differences.push(
        `⚠️ Competitor has healthier anchor profile (${competitor.healthScore} vs ${yours.healthScore})`
      );
    }
  ...
  detectOverOptimization(backlinks: BacklinkData[]): {
    ...
    if (topAnchor && topAnchor.percentage > 50) {
      score += 30;
      warnings.push(
        `🚨 Single anchor dominates profile: "${topAnchor.text}" (${topAnchor.percentage.toFixed(1)}%)`
      );
    }
