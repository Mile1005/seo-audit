 ...
 */
export async function generateLocalizedCSV(auditData: any, locale: Locale = "en"): Promise<string> {
  ...
  rows.push([t("export.page_url" as any), auditData.url]);

  rows.push([t("export.overall_score" as any), formatNumber(auditData.score, locale)]);
...
async function generatePDFSections(auditData: any, locale: Locale, t: any): Promise<PDFSection[]> {
  ...
  return checkIds.map((checkId) => {
    ...
    try {
      return {
        ...
        status: check.passed ? t("export.status_passed" as any) : t("export.status_failed" as any),
