...
export class PerformanceMonitor {
   ...
   */
  private generateRecommendations(coreWebVitals: PerformanceMetrics, pageLoad: any): string[] {
    const recommendations: string[] = [];

    // LCP recommendations
    if (coreWebVitals.lcp && coreWebVitals.lcp > 2500) {
      recommendations.push(
        "Optimize Largest Contentful Paint: Consider optimizing images, using CDN, or improving server response times"
      );
    }
    ...
    if (coreWebVitals.inp && coreWebVitals.inp > 200) {
      recommendations.push(
        "Improve Interaction to Next Paint: Reduce JavaScript execution time and optimize third-party scripts"
      );
    }
    ...
    if (coreWebVitals.cls && coreWebVitals.cls > 0.1) {
      recommendations.push(
        "Reduce Cumulative Layout Shift: Add size attributes to images and ensure fonts load properly"
      );
    }
    ...
    if (coreWebVitals.ttfb && coreWebVitals.ttfb > 600) {
      recommendations.push(
        "Improve Time to First Byte: Optimize server response times and consider using a CDN"
      );
    }
