...
function generatePerformanceOpportunities(
  ...
  if (lcp && lcp > 2.5) {
    ...
    opportunities.push("Optimize Critical Resource Priority: Ensure main content elements load first");
  ...
  if (cls && cls > 0.1) {
    ...
    opportunities.push("Specify Image Dimensions: Add width and height attributes to prevent layout shifts");
    opportunities.push("Reserve Space for Ads: Use CSS to allocate space for dynamic content");
    opportunities.push("Avoid Non-Composited Animations: Use transform and opacity for smooth animations");
  }
  
  if (inp && inp > 200) {
    opportunities.push(`Improve Interaction to Next Paint: currently ${inp}ms, target ≤200ms`);
    opportunities.push("Minimize JavaScript Execution Time: Optimize heavy scripts and event handlers");
  ...
  if (fcp && fcp > 1.8) {
    ...
    opportunities.push("Eliminate Render-Blocking Resources: Defer non-critical CSS and JavaScript");
  ...
  opportunities.push("Minify CSS and JavaScript: Reduce file sizes by removing whitespace and comments");
  opportunities.push("Enable Browser Caching: Set appropriate cache headers for static resources");
  opportunities.push("Use a Content Delivery Network (CDN): Serve assets from geographically closer servers");
  opportunities.push("Optimize Database Queries: Reduce server processing time for dynamic content");
  opportunities.push("Implement Progressive Web App Features: Add service workers for offline functionality");
  opportunities.push("Reduce HTTP Requests: Combine CSS/JS files and use CSS sprites for images");
  opportunities.push("Optimize Critical Rendering Path: Inline critical CSS and defer non-critical styles");
  ...
  opportunities.push("Optimize Mobile Performance: Ensure responsive design works efficiently on mobile devices");
...
function generatePerformanceDiagnostics(
  ...
  if (performanceScore !== null) {
    if (performanceScore >= 90) {
      ...
      diagnostics.push("Consider implementing advanced techniques like Service Workers for offline functionality");
  ...
  diagnostics.push("Evaluate loading strategies: critical resource prioritization and progressive enhancement");
  ...
  diagnostics.push("Implement performance monitoring: track metrics over time and set performance budgets");
  diagnostics.push("Optimize for perceived performance: skeleton screens and progressive loading");
  diagnostics.push("Consider modern web technologies: HTTP/2, WebAssembly for compute-intensive tasks");
  diagnostics.push("Evaluate hosting and infrastructure: server response times and geographic distribution");
