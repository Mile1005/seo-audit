...
export function useAudit(): UseAuditReturn {
  ...
  const [progress, setProgress] = useState<{ stage?: string; progress?: number; message?: string; elapsedMs?: number }>({});
  ...
  const poll = useCallback(async (auditId: string, attempt = 0) => {
    ...
    try {
      ...
      if (json.status === 'completed') {
        ...
        setLoading(false);
        setIsCached(false);
        setProgress({ stage: 'completed', progress: 100, message: 'Completed', elapsedMs: json.elapsedMs });
      ...
      if (json.status === 'failed') {
        ...
        setStatus('failed');
        setLoading(false);
        setProgress({ stage: 'failed', message: json.error || 'Audit failed', elapsedMs: json.elapsedMs });
        return;
      }
      // processing
  ...
  const loadCached = useCallback((url?: string) => {
    try {
      const normalized = url ? (normalizeUrl(url) ?? url) : localStorage.getItem(LAST_AUDIT_URL_KEY);
