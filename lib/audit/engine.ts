...
export class EnhancedAuditEngine {
  ...
  async runAudit(url: string): Promise<EnhancedAuditResult> {
    try {
      ...
      const [technicalSEO, content, security, mobile, schema] = (await Promise.all(
        coreAnalysisPromises
      )) as [TechnicalSEO, ContentAnalysis, SecurityAudit, MobileUsability, SchemaMarkup];
