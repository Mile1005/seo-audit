...
import { URL } from 'url';
import { TechnicalSEO, SecurityAudit, MobileUsability, SchemaMarkup, ContentAnalysis, AccessibilityAudit, PerformanceMetrics, CategoryScores, AuditIssue, ExecutiveSummary } from './types';
...
export class AuditAnalyzer {
  ...
  generateQuickWins(issues: AuditIssue[]): Array<{
    ...
    return issues
      .filter(issue => issue.effort === 'low' && ['critical', 'important'].includes(issue.priority))
  ...
  private calculateSEOScore(technical: TechnicalSEO, content: ContentAnalysis, schema: SchemaMarkup): number {
    let score = 100;
  ...
  private generateAccessibilityIssues(accessibility: AccessibilityAudit): AuditIssue[] {
    return accessibility.violations.map(violation => ({
      ...
      priority: violation.impact === 'critical' ? 'critical' as const : 
                violation.impact === 'serious' ? 'important' as const : 'low' as const,
  ...
  private generateKeyFindings(overallScore: number, categoryScores: CategoryScores, criticalIssues: number): string[] {
    ...
    if (lowestCategory[1] < 70) {
      findings.push(`${lowestCategory[0]} performance is the primary concern (${lowestCategory[1]}/100)`);
    }
