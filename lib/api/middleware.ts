...
export function withProjectAccess() {
  return function (handler: (req: AuthenticatedRequest, projectId: string) => Promise<NextResponse>) {
    return async (req: AuthenticatedRequest) => {
      try {
        ...
        if (req.method === 'GET') {
          const url = new URL(req.url)
          projectId = url.pathname.split('/').find((segment, index, array) => 
            array[index - 1] === 'projects' && segment !== 'projects'
          ) || ''
        } else {
          const body = await req.json()
