...
pageFiles.forEach(filePath => {
  ...
  const hasHardcodedCanonical = content.includes('canonical:') && !content.includes('generateSEOMeta');

  // Check if it's in the problematic URLs list
  const isProblematic = problematicUrls.some(url => {
    const pagePath = '/' + relativePath.replace(/\[locale\]/g, 'en').replace(/\/page\.tsx?$/, '').replace(/\\/g, '/');
    return url === pagePath;
  });
  ...
  if (isProblematic) {
    ...
    if (isClientComponent && !hasGenerateMetadata) {
      console.log(`   ❌ ISSUE: Client component without generateMetadata - no SEO metadata generated`);
      issues.push({
        file: relativePath,
