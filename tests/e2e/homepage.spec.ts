...
test.describe("Homepage Conversion Flows", () => {
  ...
  test("Hero section CTA leads to audit tool", async ({ page }) => {
    ...
    const primaryCTA = page
      .locator('[data-testid="hero-cta"], .hero-cta, button:has-text("Start Free Audit")')
      .first();
  ...
  test("Secondary CTA leads to features page", async ({ page }) => {
    const secondaryCTA = page
      .locator('a:has-text("Learn More"), a:has-text("View Features"), .secondary-cta')
      .first();
  ...
  test("Social proof elements are present", async ({ page }) => {
    ...
    const testimonials = page.locator(
      '[data-testid="testimonials"], .testimonial, .testimonials-section'
    );
  ...
  test("Newsletter signup functionality", async ({ page }) => {
    ...
    if ((await newsletterForm.count()) > 0) {
      ...
      await expect(page.locator('.success, .thank-you, [role="alert"]').first()).toBeVisible({
        timeout: 10000,
      });
    }
  });
  ...
  test("Mobile responsive behavior", async ({ page }) => {
    ...
    const mobileMenuTrigger = page
      .locator('[data-testid="mobile-menu"], .mobile-menu-trigger, .hamburger')
      .first();
    if ((await mobileMenuTrigger.count()) > 0) {
      ...
      const mobileMenu = page
        .locator('[data-testid="mobile-nav"], .mobile-nav, .mobile-menu')
        .first();
