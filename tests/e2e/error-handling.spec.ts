...
test.describe('Error Handling E2E Tests', () => {
  ...
  test('API timeout handling', async ({ page }) => {
    ...
    // Trigger API call
    const apiTrigger = page.locator(
      'button:has-text("Audit"), button:has-text("Submit"), button:has-text("Save")'
    ).first();
  ...
  test('Rate limiting error handling', async ({ page }) => {
    ...
    // Trigger API call
    const apiTrigger = page.locator(
      'button:has-text("Audit"), button:has-text("Submit")'
    ).first();
  ...
  test('Offline handling', async ({ page }) => {
    ...
    if (await link.count() > 0) {
      ...
      const offlineIndicator = page.locator(
        '.offline, .no-connection, [data-testid="offline"]'
      );
