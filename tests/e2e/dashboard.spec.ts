...
test.describe('Dashboard E2E Tests', () => {
  ...
  test('Audit history display', async ({ page }) => {
    ...
    const auditHistory = page.locator(
      '.audit-history, [data-testid="audit-history"], .recent-audits, .history'
    ).first();
    ...
    if (await auditHistory.count() > 0) {
      ...
      const auditItems = page.locator(
        '.audit-item, .audit-entry, [data-testid="audit-item"]'
      );
      
      const emptyState = page.locator(
        '.no-audits, .empty-state, :has-text("No audits")'
      );
      ...
      if (hasItems) {
        ...
        const auditScore = firstAudit.locator('.score, .audit-score, .status, [data-testid="audit-score"]');
  ...
  test('Start new audit from dashboard', async ({ page }) => {
    ...
    const newAuditTrigger = page.locator(
    ...
    if (await newAuditTrigger.count() > 0) {
      ...
      if (await urlInput.count() > 0) {
        ...
        const submitBtn = auditForm.locator('button[type="submit"], button:has-text("Audit")').first();
      ...
      const auditButton = page.locator('button:has-text("Audit"), button:has-text("Analyze")').first();
  ...
  test('Dashboard statistics and metrics', async ({ page }) => {
    ...
    const charts = page.locator(
      '.chart, .graph, canvas, svg, [data-testid="chart"]'
    );
  ...
  test('User profile and settings access', async ({ page }) => {
    ...
    if (await userMenu.count() > 0) {
      ...
      const profileLink = page.locator(
        'a:has-text("Profile"), a:has-text("Settings"), a:has-text("Account")'
      ).first();
  ...
  test('Dashboard data filtering and search', async ({ page }) => {
    ...
    const filterDropdown = page.locator(
      'select, .filter-dropdown, [data-testid="filter"]'
    ).first();
  ...
  test('Dashboard error states', async ({ page }) => {
    ...
    const retryButton = page.locator(
      'button:has-text("Retry"), button:has-text("Try Again"), .retry-btn'
    ).first();
