...
test("parser extracts all SEO elements from comprehensive HTML", () => {
  ...
  expect(parsed.metaDescription).toBe("Learn the essential SEO best practices for 2024. Discover proven strategies to improve your search rankings, drive organic traffic, and boost your website's visibility in search engines.");
  ...
  expect(parsed.images[0].alt).toBe("SEO optimization flowchart showing the relationship between technical, on-page, and off-page factors");
  expect(parsed.images[1].src).toBe("https://example.com/images/local-seo-example.png");
  expect(parsed.images[1].alt).toBe("Example of optimized Google My Business profile showing business hours, contact information, and customer reviews");
  
  // Internal links
  expect(parsed.internalLinks.length).toBeGreaterThan(10);
  expect(parsed.internalLinks.some(link => link.href === "https://example.com/technical-seo")).toBe(true);
  expect(parsed.internalLinks.some(link => link.href === "https://example.com/content-marketing")).toBe(true);
  expect(parsed.internalLinks.some(link => link.href === "https://example.com/keyword-research-guide")).toBe(true);
