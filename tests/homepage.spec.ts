...
test.describe('AISEOTurbo Homepage - Core Functionality', () => {
  ...
  test('no horizontal scrollbars at common breakpoints', async ({ page }) => {
    ...
    for (const breakpoint of breakpoints) {
      ...
      expect(hasHorizontalScroll, `Horizontal scroll detected at ${breakpoint.name} (${breakpoint.width}px)`).toBeFalsy()
    }
  })
  ...
  test('features section loads and displays cards', async ({ page }) => {
    ...
    const featuresSection = page.locator('[data-testid="features-section"], .features, section').nth(1)
  ...
  test('pricing section displays and toggle works', async ({ page }) => {
    ...
    const pricingSection = page.locator('[data-testid="pricing-section"], .pricing, section').last()
    ...
    if (priceCount > 0) {
      ...
      const billingToggle = page.locator('[data-testid="billing-toggle"], .toggle, input[type="checkbox"]').first()
  ...
  test('email capture form validation works', async ({ page }) => {
    ...
    const emailInputs = page.locator('[data-testid="lead-email"], input[type="email"], input[placeholder*="email" i]')
    ...
    if (emailCount > 0) {
      ...
      const submitButton = page.locator('[data-testid="lead-submit"], button[type="submit"], .submit-btn').first()
      if (await submitButton.isVisible()) {
        await submitButton.click()
        
        // Check for error message (flexible selectors)
        const errorVisible = await page.locator('[role="alert"], .error, .invalid, text=/invalid|valid email/i').first().isVisible({ timeout: 2000 }).catch(() => false)
        ...
        const errorStillVisible = await page.locator('[role="alert"], .error').first().isVisible({ timeout: 1000 }).catch(() => false)
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (isVisible) {
        ...
        const isInteractive = interactiveTags.includes(tagName) || 
                             await focusedElement.getAttribute('tabindex').then(val => val !== null).catch(() => false)
...
test.describe('AISEOTurbo Homepage - A/B Testing & Analytics', () => {
  ...
  test('analytics tracking attributes are present on CTAs', async ({ page }) => {
    ...
    if (trackedCount > 0) {
      ...
      const eventType = await firstTracked.getAttribute('data-event') || 
