...
const BLOG_TITLES = {
  ...
  'content-seo-creating-search-friendly-content': 'Content SEO: Creating Search-Friendly Content | AI SEO Turbo Blog',
  'core-web-vitals-optimization-guide': 'Core Web Vitals Optimization Guide - Improve Page Experience | AI SEO Turbo Blog',
...
class TitleOptimizer {
  ...
  generateOptimizedTitle(row) {
    ...
    if (pageType === 'homepage') {
      if (locale === 'en') {
        ...
        baseTitle = localizedTitles[locale] || `AI SEO Audit Tool - Boost Rankings 300% Faster | AI SEO Turbo${locationSuffix}`;
      }
    }
    // Handle other page types
      ...
      baseTitle = BLOG_TITLES[blogSlug] || `${PAGE_KEYWORDS.blog.primary} - ${PAGE_KEYWORDS.blog.secondary} | AI SEO Turbo`;
      if (locationSuffix) {
        baseTitle = baseTitle.replace(' | AI SEO Turbo Blog', `${locationSuffix} | AI SEO Turbo Blog`);
      }
    } else if (pageType.startsWith('case-studies/')) {
      const caseSlug = pageType.replace('case-studies/', '');
      baseTitle = CASE_STUDY_TITLES[caseSlug] || `${PAGE_KEYWORDS['case-studies'].primary} - ${PAGE_KEYWORDS['case-studies'].secondary} | AI SEO Turbo`;
      if (locationSuffix) {
  ...
  printSummary() {
    ...
    Object.entries(localeStats).forEach(([locale, stats]) => {
      ...
      console.log(`   ${localeName} (${locale}): ${stats.total} pages, ${stats.optimized} optimized, ${stats.errors} errors`);
    });

    // Title length validation
    const validLengths = this.csvData.filter(row =>
      row.status !== 'ERROR' && this.validateTitleLength(row.optimizedTitle)
    ).length;
    ...
    console.log(`   Average length: ${Math.round(this.csvData.filter(r => r.status !== 'ERROR').reduce((sum, r) => sum + r.optimizedTitle.length, 0) / totalValid)} characters`);
