...
class TitleErrorRecovery {
  ...
  async retryUrlWithBackoff(url, maxRetries = 3) {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      console.log(
        `   Attempt ${attempt}/${maxRetries} for: ${url.replace("https://www.aiseoturbo.com", "")}`
      );
  ...
  generateOptimizedTitle(row) {
    ...
    const actualTitle =
      recoveredTitle && recoveredTitle !== "ERROR" ? recoveredTitle : row.originalTitle;
  ...
  async recoverErrorTitles() {
    ...
    for (let i = 0; i < this.errorUrls.length; i += batchSize) {
      ...
      console.log(
        `Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(this.errorUrls.length / batchSize)} (${batch.length} URLs)`
      );
  ...
  generateUpdatedCSV() {
    const header =
      "URL,Locale,Locale Name,Original Title,Optimized Title,Recovered Title,Final Title,HTML Lang,Status,Error\n";
    ...
    const rows = this.csvData.map((row) => {
      ...
      const escapedRecovered = row.recoveredTitle
        ? `"${row.recoveredTitle.replace(/"/g, '""')}"`
        : "";
  ...
  printSummary() {
    ...
    Object.entries(localeRecovery).forEach(([locale, stats]) => {
      ...
      console.log(
        `   ${localeName} (${locale}): ${stats.recovered}/${stats.total} recovered (${rate}%)`
      );
    });
