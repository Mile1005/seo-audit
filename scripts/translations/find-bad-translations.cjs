...
function checkTranslations() {
  ...
  LOCALES.forEach(locale => {
    ...
    if (emptyKeysList.length > 0) {
      ...
      console.log(`      (Full empty list saved to scripts/translations/empty-keys-${locale}.json)`);
    }
    ...
    if (dirtyKeysList.length > 0) {
      ...
      console.log(`      (Full dirty list saved to scripts/translations/cleanup-report-${locale}.json)`);
    }
    console.log('---------------------------------------------------');
  });
}
