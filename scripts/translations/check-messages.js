...
function main() {
  ...
  for (const locale of LOCALES) {
    ...
    if (missing.length) {
      console.error(
        `[i18n] ${locale} missing ${missing.length} keys:\n - ${missing.slice(0, 20).join("\n - ")}${missing.length > 20 ? "\n   ..." : ""}`
      );
      errors++;
    }
    if (extra.length) {
      console.warn(
        `[i18n] ${locale} has ${extra.length} extra keys (ok):\n - ${extra.slice(0, 10).join("\n - ")}${extra.length > 10 ? "\n   ..." : ""}`
      );
    }
    ...
    if (hasServerChecks) {
      ...
      if (missingChecks.length) {
        console.error(
          `[i18n] ${locale} missing audit check translations (${missingChecks.length}):\n - ${missingChecks.slice(0, 20).join("\n - ")}${missingChecks.length > 20 ? "\n   ..." : ""}`
        );
        errors++;
      }
    }

    const hasServerNotifications = Object.keys(baseFlat).some((k) =>
      k.startsWith("server.notifications.")
    );
    if (hasServerNotifications) {
      ...
      if (!title || !/\{\s*domain\s*\}/.test(title)) {
        console.warn(
          `[i18n] ${locale} notification title may miss {domain} placeholder: ${titleKey}`
        );
      }
      if (!message || !/\{\s*score\s*\}/.test(message)) {
        console.warn(
          `[i18n] ${locale} notification message may miss {score} placeholder: ${msgKey}`
        );
      }
    }
  }
