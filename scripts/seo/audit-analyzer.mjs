...
class SEOAuditAnalyzer {
  ...
  analyzeIssues() {
    ...
    const checks = ['hierarchy', 'titles', 'descriptions', 'metaTags', 'socialGraphs', 'schemas', 'hreflang', 'canonical', 'urlFormat', 'technical', 'content', 'performance', 'accessibility'];
  ...
  calculateAverageScore() {
    ...
    this.results.pages.forEach(page => {
      ...
      Object.values(page.checks).forEach(check => {
        checkCount++;
        switch (check.status) {
          case 'passed': pageScore += 100; break;
          case 'warning': pageScore += 50; break;
          case 'failed': pageScore += 0; break;
          default: pageScore += 25; break;
        }
      });
  ...
  showCriticalIssues() {
    ...
    console.log(`   ${this.getIssueCount('high', 'not_self_referencing')} pages have incorrect canonical URLs`);
    console.log(`   ${this.getIssueCount('high', 'double_slashes')} pages have double slashes in URLs`);
  }
  ...
  showCheckBreakdown() {
    ...
    Object.entries(this.checkStats).forEach(([check, stats]) => {
      ...
      console.log(`   ${status} ${check.padEnd(15)}: ${stats.passed} passed, ${stats.warning} warnings, ${stats.failed} failed (${passRate}% pass rate)`);
    });
  }
  ...
  showGlobalIssues() {
    ...
    if (this.results.globalIssues.duplicateTitles.length > 0) {
      console.log(`   📝 ${this.results.globalIssues.duplicateTitles.length} duplicate title groups found`);
    }

    if (this.results.globalIssues.duplicateDescriptions.length > 0) {
      console.log(`   📝 ${this.results.globalIssues.duplicateDescriptions.length} duplicate description groups found`);
    }
  ...
  showRecommendations() {
    ...
    if (this.getIssueCount('critical', 'multiple') > 0) {
      console.log(`   🔥 PRIORITY 1: Fix multiple title tags (${this.getIssueCount('critical', 'multiple')} pages)`);
    ...
    if (this.getIssueCount('critical', 'missing') > 0) {
      console.log(`   🔥 PRIORITY 2: Add missing title tags (${this.getIssueCount('critical', 'missing')} pages)`);
    ...
    if (this.getIssueCount('high', 'missing') > 0) {
      console.log(`   ⚡ PRIORITY 3: Add missing meta descriptions (${this.getIssueCount('high', 'missing')} pages)`);
    ...
    if (this.getIssueCount('high', 'double_slashes') > 0) {
      console.log(`   🔗 PRIORITY 4: Fix URL formatting issues (${this.getIssueCount('high', 'double_slashes')} pages)`);
    ...
    if (this.getIssueCount('high', 'not_self_referencing') > 0) {
      console.log(`   ⚡ PRIORITY 5: Fix canonical URLs (${this.getIssueCount('high', 'not_self_referencing')} pages)`);
