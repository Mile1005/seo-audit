...
class PageHealthChecker {
  ...
  async testAllPages() {
    ...
    console.log(`Testing ${COLORS.bright}${testUrls.length}${COLORS.reset} URLs (${uniquePages.length} routes × locales)...\n`);
    ...
    for (let i = 0; i < testUrls.length; i += batchSize) {
      ...
      process.stdout.write(`\r${COLORS.cyan}Progress: ${completed}/${testUrls.length} (${Math.round(completed/testUrls.length*100)}%)${COLORS.reset}`);
    }
  ...
  calculateStats() {
    const allResults = [...this.results.success, ...this.results.notFound, ...this.results.errors, ...this.results.redirects];
  ...
  printReport() {
    ...
    console.log(`  ├─ ${COLORS.green}✓ Success (200-299):${COLORS.reset}  ${this.results.success.length}`);
    console.log(`  ├─ ${COLORS.yellow}↻ Redirects (300-399):${COLORS.reset} ${this.results.redirects.length}`);
    console.log(`  ├─ ${COLORS.red}✗ Not Found (404):${COLORS.reset}    ${this.results.notFound.length}`);
    console.log(`  └─ ${COLORS.red}⚠ Errors:${COLORS.reset}            ${this.results.errors.length}`);
    console.log();
    console.log(`Average Response Time: ${COLORS.bright}${this.stats.avgResponseTime}ms${COLORS.reset}`);
    console.log(`Total Content Size:    ${COLORS.bright}${(this.stats.totalSize / 1024 / 1024).toFixed(2)} MB${COLORS.reset}`);
    console.log();

    // Success pages
    if (this.results.success.length > 0) {
      console.log(COLORS.bright + COLORS.green + `\n✅ SUCCESSFUL PAGES (${this.results.success.length})` + COLORS.reset);
      console.log('─'.repeat(100));
      ...
      this.results.success.slice(0, 20).forEach(result => {
        ...
        const size = result.size ? `${(result.size / 1024).toFixed(1)}KB`.padEnd(9) : 'N/A'.padEnd(9);
        console.log(`${COLORS.green}${status}${COLORS.reset}  ${time} ${size} ${result.url}`);
      });
      
      if (this.results.success.length > 20) {
        console.log(COLORS.yellow + `\n... and ${this.results.success.length - 20} more successful pages` + COLORS.reset);
      }
    }

    // Redirects
    if (this.results.redirects.length > 0) {
      console.log(COLORS.bright + COLORS.yellow + `\n↻ REDIRECTS (${this.results.redirects.length})` + COLORS.reset);
      console.log('─'.repeat(100));
    ...
    if (this.results.notFound.length > 0) {
      console.log(COLORS.bright + COLORS.red + `\n❌ NOT FOUND (404) - ${this.results.notFound.length} pages` + COLORS.reset);
      console.log('─'.repeat(100));
    ...
    if (this.results.errors.length > 0) {
      console.log(COLORS.bright + COLORS.red + `\n⚠️  ERRORS - ${this.results.errors.length} pages` + COLORS.reset);
      console.log('─'.repeat(100));
    ...
    if (this.results.notFound.length > 0) {
      console.log(COLORS.red + `• Fix ${this.results.notFound.length} pages returning 404` + COLORS.reset);
    }
    if (this.results.errors.length > 0) {
      console.log(COLORS.red + `• Investigate ${this.results.errors.length} pages with errors` + COLORS.reset);
    }
    ...
    if (slowPages.length > 0) {
      console.log(COLORS.yellow + `• ${slowPages.length} pages have response time > 1s (consider optimization)` + COLORS.reset);
    }
