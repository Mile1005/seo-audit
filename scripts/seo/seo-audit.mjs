...
class SEOAuditor {
  ...
  checkHreflang(html, url, currentLocale) {
    ...
    if (foundLocales.size >= 5 && hasXDefault) {
      passed.push(`✓ Hreflang: ${foundLocales.size + 1} alternates (${foundLocales.size} locales + x-default)`);
    }
      ...
      try {
        ...
        if (data['@language'] || data.inLanguage) {
          ...
          if (lang !== locale && lang !== `${locale}`) {
            warnings.push(`Structured data language "${lang}" doesn't match page locale "${locale}"`);
          }
        }
  ...
  printSummary() {
    ...
    Object.entries(this.checks).forEach(([key, stats]) => {
      ...
      console.log(`${checkNames[key].padEnd(25)} ${color}${passRate}%${COLORS.reset} (${stats.pass}/${total}) - ${stats.fail} errors, ${stats.warn} warnings`);
    });
...
Promise.all(
  samplePages.map(({ path, locale }) =>
    auditor.auditPage(`${BASE_URL}${path}`, locale)
  )
)
  .then(async (results) => {
    results.forEach(result => {
      ...
      console.log(COLORS.bright + `\n${result.url}` + COLORS.reset + COLORS.cyan + ` [${result.locale}]` + COLORS.reset);
      
      if (result.passed.length > 0) {
        console.log(COLORS.green + result.passed.join('\n') + COLORS.reset);
      }
      if (result.warnings.length > 0) {
        console.log(COLORS.yellow + result.warnings.map(w => `⚠ ${w}`).join('\n') + COLORS.reset);
      }
