  ...
  async diagnosePage(url) {
    ...
    try {
      ...
      if (headMatch) {
        ...
        console.log(
          headContent.substring(0, 2000) + (headContent.length > 2000 ? "\n... (truncated)" : "")
        );
        ...
        descMatches.forEach((descTag, index) => {
          ...
          console.log(
            `   ${index + 1}. ${descContent ? descContent[1].substring(0, 100) + "..." : "NO CONTENT"}`
          );
        });

        // Count canonical tags
        const canonicalMatches =
          headContent.match(/<link[^>]+rel=["']canonical["'][^>]+href=["']([^"']+)["'][^>]*>/gi) ||
          [];
