...
class UltimateSEOAuditor {
  ...
  async auditPage(url) {
    ...
    try {
      ...
      if (response.status >= 300 && response.status < 400) {
        ...
        audit.checks.titles.details.note =
          "Skipped due to redirect - cannot verify final page content";
        audit.checks.descriptions.status = "skipped";
        audit.checks.descriptions.details.note =
          "Skipped due to redirect - cannot verify final page content";
        audit.checks.hierarchy.status = "skipped";
        audit.checks.hierarchy.details.note =
          "Skipped due to redirect - cannot verify final page content";
        audit.checks.metaTags.status = "skipped";
        audit.checks.metaTags.details.note =
          "Skipped due to redirect - cannot verify final page content";
        audit.checks.socialGraphs.status = "skipped";
        audit.checks.socialGraphs.details.note =
          "Skipped due to redirect - cannot verify final page content";
        audit.checks.schemas.status = "skipped";
        audit.checks.schemas.details.note =
          "Skipped due to redirect - cannot verify final page content";
        audit.checks.content.status = "skipped";
        audit.checks.content.details.note =
          "Skipped due to redirect - cannot verify final page content";
        audit.checks.accessibility.status = "skipped";
        audit.checks.accessibility.details.note =
          "Skipped due to redirect - cannot verify final page content";
      ...
      const hasReactScripts =
        /<script[^>]*src[^>]*react[^>]*>|<script[^>]*src[^>]*next[^>]*>/i.test(html) ||
      ...
      if (hasReactScripts) {
        ...
        try {
          ...
          this.log(
            COLORS.red,
            `❌ Puppeteer failed for ${url}: ${puppeteerError.message}, using initial HTML`
          );
      ...
      Object.entries(weights).forEach(([checkType, weight]) => {
        ...
        if (check.issues) {
          const highSeverity = check.issues.filter(
  ...
  async checkSEOHierarchy(html, audit) {
    ...
    if (audit.checks.hierarchy.h1Count === 0) {
      ...
      audit.checks.hierarchy.issues.push(
        `Multiple H1 tags found: ${audit.checks.hierarchy.h1Count}`
      );
    }
    ...
    if (audit.checks.hierarchy.h2Count > 4) {
      audit.checks.hierarchy.status = "warning";
      audit.checks.hierarchy.issues.push(
        `Many H2 tags found: ${audit.checks.hierarchy.h2Count} (consider consolidating)`
      );
    }
    ...
    headings.forEach((heading, index) => {
      if (heading.level > lastLevel + 1 && lastLevel !== 0) {
        hierarchyIssues.push(
          `Hierarchy skip: H${lastLevel} -> H${heading.level} at "${heading.text}"`
        );
      }
  ...
  checkTitlesAndDescriptions(html, audit) {
    ...
    if (!titleMatch) {
      ...
      audit.checks.titles.details.quality = {
        hasBrand:
          title.toLowerCase().includes("aiseoturbo") ||
          title.toLowerCase().includes("ai seo turbo"),
    ...
    const htmlTitleTags = titleTags
      ? titleTags.filter(
          (tag) =>
            !tag.includes("<svg") &&
            !tag.includes("<path") &&
            !html.substring(html.indexOf(tag) - 100, html.indexOf(tag)).includes("<svg")
        )
      : [];
    ...
    if (!descMatch) {
      ...
      audit.checks.descriptions.details.quality = {
        hasCallToAction: /\b(click|learn|discover|get|start|try|see|view|read)\b/i.test(
          description
        ),
  ...
  checkMetaTags(html, audit) {
    ...
    if (issues.length > 0) {
      audit.checks.metaTags.status = issues.some((i) => i.severity === "critical")
        ? "failed"
        : "warning";
    } else {
  ...
  checkUrlFormat(url, audit) {
    ...
    if (issues.length > 0) {
      audit.checks.urlFormat.status = issues.some(
        (i) => i.severity === "critical" || i.severity === "high"
      )
        ? "failed"
        : "warning";
  ...
  checkSocialGraphs(html, audit) {
    ...
    const ogTags = {
      ...
      "og:locale": html.match(
        /<meta[^>]+property=["']og:locale["'][^>]+content=["']([^"']+)["'][^>]*>/i
      ),
      "og:site_name": html.match(
    ...
    Object.entries(ogTags).forEach(([tag, match]) => {
      if (match) {
        ...
        details.ogTags[tag] = {
          ...
          isAbsolute:
            tag.includes("url") || tag.includes("image") ? match[1].startsWith("http") : true,
        };
      }
    });
    ...
    const twitterTags = {
      "twitter:card": html.match(
        /<meta[^>]+name=["']twitter:card["'][^>]+content=["']([^"']+)["'][^>]*>/i
      ),
      "twitter:title": html.match(
        /<meta[^>]+name=["']twitter:title["'][^>]+content=["']([^"']+)["'][^>]*>/i
      ),
      "twitter:description": html.match(
        /<meta[^>]+name=["']twitter:description["'][^>]+content=["']([^"']+)["'][^>]*>/i
      ),
      "twitter:image": html.match(
        /<meta[^>]+name=["']twitter:image["'][^>]+content=["']([^"']+)["'][^>]*>/i
      ),
      "twitter:site": html.match(
        /<meta[^>]+name=["']twitter:site["'][^>]+content=["']([^"']+)["'][^>]*>/i
      ),
      "twitter:creator": html.match(
    ...
    if (details.totalTags === 0) {
      ...
      if (
        details.twitterTags["twitter:card"] &&
        !["summary", "summary_large_image", "app", "player"].includes(
          details.twitterTags["twitter:card"].content
        )
      ) {
        issues.push({
    ...
    if (issues.length > 0) {
      audit.checks.socialGraphs.status = issues.some((i) => i.severity === "high")
        ? "failed"
        : "warning";
    } else if (details.totalTags >= 8) {
      // Good coverage
  ...
  checkCanonical(html, url, audit) {
    ...
    try {
      ...
      details.isSelfReferencing =
        urlObj.pathname === canonicalObj.pathname && urlObj.search === canonicalObj.search;
  ...
  checkContentQuality(html, audit) {
    ...
    const readabilityScore =
      206.835 - 1.015 * avgWordsPerSentence - 84.6 * (complexWords / details.wordCount);
    ...
    if (issues.length > 0) {
      audit.checks.content.status = issues.some((i) => i.severity === "high")
        ? "failed"
        : "warning";
  ...
  checkAccessibility(html, audit) {
    ...
    if (issues.length > 0) {
      audit.checks.accessibility.status = issues.some((i) => i.severity === "high")
        ? "failed"
        : "warning";
  ...
  printSummary() {
    ...
    Object.entries(checkSummary).forEach(([check, stats]) => {
      ...
      console.log(
        `${check.padEnd(15)}: ✅ ${stats.passed} ⚠️ ${stats.warning} ❌ ${stats.failed} (${passRate}% pass, ${failRate}% fail)`
      );
    });
    ...
    const avgWordCount =
      this.results.pages.reduce((sum, p) => sum + (p.checks.content.details.wordCount || 0), 0) /
      this.results.pages.length;
    const avgReadingTime =
      this.results.pages.reduce((sum, p) => sum + (p.checks.content.details.readingTime || 0), 0) /
      this.results.pages.length;
    ...
    console.log(
      `🎯 Pages with sufficient content: ${this.results.pages.filter((p) => (p.checks.content.details.wordCount || 0) >= 300).length}/${this.results.pages.length}`
    );
...
async function getUrlsToAudit() {
  try {
    ...
    const workingUrls = results.pages.filter((p) => p.status === 200).map((p) => p.url);
    ...
    if (sitemapResponse.ok) {
      ...
      try {
        ...
        const validatedData = JSON.parse(
          await fs.readFile(join(rootDir, "sitemap-valid-urls.json"), "utf8")
        );
        if (validatedData.validUrls && validatedData.validUrls.length > 0) {
          console.log(
            `Using ${validatedData.validUrls.length} pre-validated URLs from sitemap-valid-urls.json`
          );
