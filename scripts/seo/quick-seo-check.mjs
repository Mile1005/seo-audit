...
async function checkPage(url) {
  ...
  try {
    ...
    if (schemasWithLanguage.length > 0) {
      console.log(`     Languages found: ${schemasWithLanguage.map(s => s['@language']).join(', ')}`);
    }
...
async function runAudit() {
  ...
  if (validResults.length > 0) {
    const avgSchemasWithLanguage = validResults.reduce((sum, r) => sum + r.schemasWithLanguage, 0) / validResults.length;
