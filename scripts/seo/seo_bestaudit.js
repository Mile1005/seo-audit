...
function checkTitle(title, keywords, allTitles) {
  ...
  if (title.length < 50 || title.length > 60)
    return { status: "warn", score: 50, notes: `Title length ${title.length}` };
  const primaryKeyword = keywords[0] || "";
  if (primaryKeyword && !title.toLowerCase().includes(primaryKeyword.toLowerCase()))
    return { status: "warn", score: 70, notes: "Primary keyword not in title" };
...
function checkMetaDesc(desc, allDescriptions) {
  ...
  if (desc.length < 120 || desc.length > 155)
    return { status: "warn", score: 50, notes: `Description length ${desc.length}` };
...
function checkCanonical($, url) {
  ...
  if (!canonical || canonical !== url)
    return { status: "fail", score: 0, notes: "Canonical missing or incorrect" };
  return { status: "pass", score: 100, notes: "Good" };
}
...
async function checkHreflang($, url, lang) {
  ...
  if (!hreflangs[lang] || hreflangs[lang] !== url)
    return { status: "fail", score: 0, notes: "Self hreflang missing or incorrect" };
  const missing = LANGUAGES.filter((l) => !hreflangs[l]);
  if (missing.length)
    return { status: "warn", score: 50, notes: `Missing hreflang for ${missing.join(",")}` };
  ...
  if (altUrl) {
    try {
      ...
      if (!altHreflangs[lang] || altHreflangs[lang] !== url)
        return { status: "warn", score: 60, notes: "Bidirectional hreflang missing" };
    } catch (e) {
...
function checkOpenGraph($) {
  ...
  if (!ogTitle || !ogDesc || !ogImage || !ogUrl || !ogType)
    return { status: "fail", score: 0, notes: "Missing OG tags" };
  return { status: "pass", score: 100, notes: "Good" };
}
...
function checkAria($) {
  ...
  if (missingAlt > 0)
    return { status: "warn", score: 50, notes: `${missingAlt} images missing alt` };
  return { status: "pass", score: 100, notes: "Good" };
}
...
function checkKeywords($, keywords) {
  ...
  if (density > 2)
    return { status: "warn", score: 50, notes: `Keyword density ${density.toFixed(2)}%` };
  return { status: "pass", score: 100, notes: "Good" };
}
...
function checkLinks($) {
  ...
  if (internals < 3)
    return { status: "warn", score: 50, notes: `Only ${internals} internal links` };
  return { status: "pass", score: 100, notes: "Good" };
}
...
function checkMobile($) {
  ...
  if (!viewport || !viewport.includes("width=device-width"))
    return { status: "fail", score: 0, notes: "No viewport" };
  return { status: "pass", score: 100, notes: "Good" };
}
    ...
    try {
      ...
      const scripts = $("script[src]")
        ...
        .some(
          (s) =>
            $(s).attr("src") &&
            ($(s).attr("src").includes("react") || $(s).attr("src").includes("next"))
        );
