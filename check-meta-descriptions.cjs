...
async function checkPage(locale) {
  ...
  try {
    ...
    if (meta.h1) {
      ...
      if (h1Length < 20) {
        console.log(`   ⚠️  WARNING: H1 is too short (${h1Length} chars). Google recommends 20-70 characters.`);
      } else if (h1Length > 70) {
        console.log(`   ⚠️  WARNING: H1 is too long (${h1Length} chars). Google recommends 20-70 characters.`);
      } else {
    ...
    const hasEnglishWords = (text) => {
      ...
      const englishIndicators = page === 'pricing'
  ...
  try {
    ...
    const hasEnglishWords = (text, pageType) => {
      ...
      const indicators = {
        ...
        'seo-tools': ['Explore our comprehensive', 'SEO tools', 'boost your search', 'competitor analysis']
      };
...
async function main() {
  ...
  for (const pageType of pagesToCheck) {
    ...
    if (uniqueDescriptions.length > 1) {
      ...
      console.log('   ⚠️  WARNING: All descriptions are identical - may indicate missing translations');
    }
  }
