    ...
    >
      ...
      {result.issues && result.issues.length > 0 && (
        <section className="mt-8">
          <h2 className="text-xl font-bold mb-4 text-accent-primary">Issues Found</h2>
          <ul className="space-y-4">
            {result.issues.map((issue, idx) => (
              <li key={idx} className="p-4 rounded-lg border border-red-400/30 bg-red-400/10">
                <div className="font-semibold text-red-500 mb-1">{issue.found}</div>
                <div className="text-sm text-text-secondary mb-1"><strong>Severity:</strong> {issue.severity}</div>
                <div className="text-sm text-text-secondary mb-1"><strong>Category:</strong> {issue.category}</div>
                <div className="text-sm text-text-secondary mb-1"><strong>Why it matters:</strong> {issue.why_it_matters}</div>
                <div className="text-sm text-accent-primary mb-1"><strong>Recommendation:</strong> {issue.recommendation}</div>
                {issue.snippet && (
                  <pre className="bg-bg-secondary/50 p-2 rounded text-xs mt-2 overflow-x-auto"><code>{issue.snippet}</code></pre>
                )}
              </li>
            ))}
          </ul>
      ...
      {result.accessibility_issues && result.accessibility_issues.length > 0 && (
        <section className="mt-8">
          <h2 className="text-xl font-bold mb-4 text-blue-500">Accessibility & ARIA Issues</h2>
          <ul className="space-y-4">
            {result.accessibility_issues.map((issue, idx) => (
              <li key={idx} className="p-4 rounded-lg border border-blue-400/30 bg-blue-400/10">
                <div className="font-semibold text-blue-500 mb-1">{issue.message}</div>
                <div className="text-xs text-text-secondary mb-1"><strong>Type:</strong> {issue.type}</div>
                <div className="text-xs text-text-secondary mb-1"><strong>Selector:</strong> <span className="font-mono">{issue.selector}</span></div>
                {issue.snippet && (
                  <pre className="bg-bg-secondary/50 p-2 rounded text-xs mt-2 overflow-x-auto"><code>{issue.snippet}</code></pre>
                )}
              </li>
      ...
      {result.quick_wins && result.quick_wins.length > 0 && (
        <section className="mt-8">
          <h2 className="text-xl font-bold mb-4 text-green-500">Quick Wins</h2>
          <ul className="space-y-4">
            {result.quick_wins.map((win, idx) => (
              <li key={idx} className="p-4 rounded-lg border border-green-400/30 bg-green-400/10">
                <div className="font-semibold text-green-600 mb-1">{win.action}</div>
                <div className="text-xs text-text-secondary mb-1"><strong>Estimated Impact:</strong> {win.estimated_impact}</div>
                {win.snippet && (
                  <pre className="bg-bg-secondary/50 p-2 rounded text-xs mt-2 overflow-x-auto"><code>{win.snippet}</code></pre>
                )}
              </li>
            ))}
          </ul>
