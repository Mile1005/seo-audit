...
export function KeywordVariationsModal({
  ...
  const allVariations = useMemo(
    () => generateVariations(),
    [baseKeyword, baseVolume, baseDifficulty]
  );
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col animate-in fade-in zoom-in duration-300">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-slate-600 font-medium mt-1">
                <span className="text-yellow-700 font-bold">{filteredVariations.length}</span>{" "}
                variations for "<span className="font-semibold">{baseKeyword}</span>"
              </p>
            </div>
                    ...
                    <div className="flex items-center gap-4 flex-wrap">
                      <span className="text-sm font-medium text-slate-600 flex items-center gap-1">
                        ...
                        <span className="font-bold text-blue-700">
                          {variation.volume.toLocaleString()}
                        </span>{" "}
                        /mo
                      </span>
                      <span className="text-sm font-medium text-slate-600 flex items-center gap-1">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-bold text-slate-700">
                          {Math.round(variation.difficulty)}%
                        </span>
                      </div>
                    </div>
                  </div>
                  <Badge
                    className={`px-3 py-1.5 font-bold border-0 whitespace-nowrap ${getDifficultyColor(variation.difficulty)}`}
                  >
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between">
          <p className="text-sm text-slate-600">
            Showing <span className="font-bold text-slate-900">{filteredVariations.length}</span> of{" "}
            <span className="font-bold text-slate-900">{allVariations.length}</span> variations
          </p>
