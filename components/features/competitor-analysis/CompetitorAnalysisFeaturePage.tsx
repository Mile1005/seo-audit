...
const SERPComparison = dynamic(
  () => import("@/components/features/competitor-analysis/serp-comparison"),
  { ssr: false }
);
const GapAnalysis = dynamic(
  () => import("@/components/features/competitor-analysis/gap-analysis"),
  { ssr: false }
);
const MonitoringDashboard = dynamic(
  () => import("@/components/features/competitor-analysis/monitoring-dashboard"),
  { ssr: false }
);
const StrategyRecommendations = dynamic(
  () => import("@/components/features/competitor-analysis/strategy-recommendations"),
  { ssr: false }
);
const CompetitorAnalysisHero = dynamic(
  () =>
    import("@/components/features/competitor-analysis/competitor-analysis-hero").then((mod) => ({
      default: mod.CompetitorAnalysisHero,
    })),
  { ssr: false }
);
            ...
            >
              <div className="max-w-6xl mx-auto">
                <div className="text-center mb-12">
                  <h2 className="text-3xl font-bold text-foreground mb-4">{t("results.title")}</h2>
                  ...
                  <div className="bg-card rounded-xl p-6 border">
                    <div className="overflow-x-auto">
                      <table className="w-full">
                        ...
                        <tbody>
                          {analysisResults.analysis.serpComparison.map(
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="bg-card rounded-xl p-6 border">
                      ...
                      <div className="space-y-3">
                        {analysisResults.analysis.gapAnalysis.keywordGaps
                          .slice(0, 5)
                          .map((gap: any, index: number) => (
                            ...
                            >
                              <div>
                                ...
                                <p className="text-sm text-muted-foreground">
                                  Volume: {gap.volume.toLocaleString()} | Difficulty:{" "}
                                  {gap.difficulty}%
                                </p>
                              </div>
                              <span
                    ...
                    <div className="bg-card rounded-xl p-6 border">
                      ...
                      <div className="space-y-3">
                        {analysisResults.analysis.gapAnalysis.contentGaps.map(
                  ...
                  <div className="bg-card rounded-xl p-6 border">
                    <div className="overflow-x-auto">
                      <table className="w-full">
                        ...
                        <tbody>
                          {analysisResults.analysis.backlinkAnalysis.map(
                            (competitor: any, index: number) => (
                              <tr key={index} className="border-b border-muted">
                                <td className="py-3 px-4 font-medium">{competitor.domain}</td>
                                <td className="py-3 px-4 text-center">
                                  {competitor.totalBacklinks.toLocaleString()}
                                </td>
                                <td className="py-3 px-4 text-center">
                                ...
                                <td className="py-3 px-4 text-center">
                                  <span
                                    className={`px-2 py-1 rounded text-sm font-medium ${
                                      competitor.domainAuthority >= 80
                                        ? "bg-green-100 text-green-800"
                                        : competitor.domainAuthority >= 60
                                          ? "bg-yellow-100 text-yellow-800"
                                          : "bg-red-100 text-red-800"
                                    }`}
                                  >
