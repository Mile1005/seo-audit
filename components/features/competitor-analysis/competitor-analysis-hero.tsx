...
import { ArrowRight, CheckCircle, Target, Zap, TrendingUp, Shield, Play, ChevronRight, Clock, BarChart, Users, Star, AlertTriangle, Loader2, Search, Globe, Link, ExternalLink, FileText, Image, Zap as ZapIcon, TrendingDown, TrendingUp as TrendingUpIcon, Minus, Eye, Lightbulb, Trophy } from 'lucide-react'
                ...
                {keyMetrics.map((metric, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`text-xs font-medium ${metric.change.startsWith('+') ? 'text-emerald-400' : 'text-rose-400'}`}>
                        {metric.change}
                      </div>
                    </motion.div>
                  );
                    ...
                    {competitorData.map((competitor, index) => (
                      <motion.div
                        ...
                        className={`flex items-center justify-between p-4 rounded-lg transition-colors ${
                          competitor.name === 'yoursite.com' ? 'bg-primary/10 border border-primary/20' : 'bg-muted/50 hover:bg-muted'
                        }`}
                      >
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            ...
                            <div className="text-sm text-muted-foreground">{competitor.keywords} keywords</div>
                          </div>
                        </div>

                        <div className="flex items-center space-x-6">
                          <div className="text-center">
                            <div className="text-lg font-bold text-foreground">{competitor.visibility}%</div>
                            <div className="text-xs text-muted-foreground">{tHero('dashboard.visibility')}</div>
                          </div>

                          <div className="text-center">
                            <div className={`text-sm font-medium ${getTrendColor(competitor.trend)}`}>
                              {getTrendIcon(competitor.trend)}
                            </div>
                            <div className="text-xs text-muted-foreground">{tHero('dashboard.trend')}</div>
                          </div>
                        </div>
                      </motion.div>
                    ))}
                  ...
                  <div className="mt-6 pt-4 border-t">
                    <div className="grid grid-cols-3 gap-4 text-center">
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">{tHero('dashboard.gained')}</div>
                      </div>
                      <div>
                        <div className="text-lg font-bold text-rose-400">-8</div>
                        <div className="text-xs text-muted-foreground">{tHero('dashboard.lost')}</div>
                      </div>
                      <div>
                        <div className="text-lg font-bold text-blue-400">156</div>
                        <div className="text-xs text-muted-foreground">{tHero('dashboard.opportunities')}</div>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 bg-muted/30">
        <div className="max-w-4xl mx-auto">
          ...
          >
            ...
            <p className="text-lg text-muted-foreground">Enter your domain to discover competitors and uncover opportunities</p>
          </motion.div>
          ...
          >
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-medium text-foreground mb-2">Your Domain</label>
                  <input
                    type="text"
                    value={domain}
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground mb-2">Known Competitors (Optional)</label>
                  <textarea
                    value={competitors}
                    ...
                    rows={3}
                  />
                  <p className="text-xs text-muted-foreground mt-1">One domain per line. We'll find competitors automatically if not specified.</p>
                </div>
              </div>
