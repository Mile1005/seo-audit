...
const SEOAuditHero = dynamic(() => import('@/components/features/seo-audit/seo-audit-hero').then(mod => ({ default: mod.SEOAuditHero })), { ssr: false })
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 pt-8">
          <Breadcrumbs
            items={[
              ...
              { name: t('breadcrumbs.seoAudit'), url: 'https://www.aiseoturbo.com/features/seo-audit' }
            ]}
            className="mb-4"
          />
        </div>
              ...
              <div className="max-w-6xl mx-auto">
                ...
                >
                  <div className={`inline-flex items-center px-6 py-3 rounded-full text-lg font-semibold mb-4 ${
                    auditResult.score >= 80 ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' :
                    auditResult.score >= 60 ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300' :
                  ...
                  <p className="text-xl text-muted-foreground">
                    {t('results.completedOn')} {new Date(auditResult.timestamp).toLocaleDateString()}
                  </p>
                </motion.div>
                ...
                {auditResult.rawData && (
                  <>
                    ...
                    {(auditResult.rawData.scores || auditResult.comprehensiveResults?.scores) && (
                      ...
                      >
                        <h3 className="text-2xl font-semibold text-foreground mb-6">{t('results.scoresBreakdown')}</h3>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                          <div className="text-center p-4 bg-blue-500/10 rounded-lg">
                            <div className="text-3xl font-bold text-blue-600">
                              {(auditResult.rawData.scores?.overall || auditResult.comprehensiveResults?.scores?.overall || auditResult.score) || 'N/A'}
                            </div>
                            <div className="text-sm text-muted-foreground">{t('results.overallScore')}</div>
                          </div>
                          <div className="text-center p-4 bg-green-500/10 rounded-lg">
                            <div className="text-3xl font-bold text-green-600">
                              {(auditResult.rawData.scores?.performance || auditResult.comprehensiveResults?.scores?.performance) || 'N/A'}
                            </div>
                            <div className="text-sm text-muted-foreground">{t('results.performance')}</div>
                          </div>
                          <div className="text-center p-4 bg-purple-500/10 rounded-lg">
                            <div className="text-3xl font-bold text-purple-600">
                              {(auditResult.rawData.scores?.accessibility || auditResult.comprehensiveResults?.scores?.accessibility) || 'N/A'}
                            </div>
                            <div className="text-sm text-muted-foreground">{t('results.accessibility')}</div>
                          </div>
                          <div className="text-center p-4 bg-orange-500/10 rounded-lg">
                            <div className="text-3xl font-bold text-orange-600">
                              {(auditResult.rawData.scores?.seo || auditResult.comprehensiveResults?.scores?.seo) || 'N/A'}
                            </div>
                    ...
                    {/* Performance Metrics */}
                    {(auditResult.rawData.performance_metrics || auditResult.comprehensiveResults?.performance_metrics) && (
                      <motion.div
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                        ...
                        className="bg-card rounded-2xl shadow-xl p-8 mb-8 border"
                      >
                        <h3 className="text-2xl font-semibold text-foreground mb-6">{t('results.performanceMetrics')}</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                          <div className="text-center p-4 bg-muted/30 rounded-lg">
                            <div className="text-2xl font-bold text-blue-600">
                              {((auditResult.rawData.performance_metrics?.first_contentful_paint || auditResult.comprehensiveResults?.performance_metrics?.first_contentful_paint))?.toFixed(1)}s
                            </div>
                            <div className="text-sm text-muted-foreground">{t('results.firstContentfulPaint')}</div>
                            <div className="text-xs text-muted-foreground mt-1">{t('results.firstContentfulPaintDesc')}</div>
                          </div>
                          <div className="text-center p-4 bg-muted/30 rounded-lg">
                            <div className="text-2xl font-bold text-green-600">
                              {((auditResult.rawData.performance_metrics?.largest_contentful_paint || auditResult.comprehensiveResults?.performance_metrics?.largest_contentful_paint))?.toFixed(1)}s
                            </div>
                            <div className="text-sm text-muted-foreground">{t('results.largestContentfulPaint')}</div>
                            <div className="text-xs text-muted-foreground mt-1">{t('results.largestContentfulPaintDesc')}</div>
                          </div>
                          <div className="text-center p-4 bg-muted/30 rounded-lg">
                            <div className="text-2xl font-bold text-orange-600">
                              {(auditResult.rawData.performance_metrics?.total_blocking_time || auditResult.comprehensiveResults?.performance_metrics?.total_blocking_time)}ms
                            </div>
                            <div className="text-sm text-muted-foreground">{t('results.totalBlockingTime')}</div>
                            <div className="text-xs text-muted-foreground mt-1">{t('results.totalBlockingTimeDesc')}</div>
                          </div>
                          <div className="text-center p-4 bg-muted/30 rounded-lg">
                            <div className="text-2xl font-bold text-purple-600">
                              {((auditResult.rawData.performance_metrics?.cumulative_layout_shift || auditResult.comprehensiveResults?.performance_metrics?.cumulative_layout_shift))?.toFixed(3)}
                            </div>
                            <div className="text-sm text-muted-foreground">{t('results.cumulativeLayoutShift')}</div>
                            <div className="text-xs text-muted-foreground mt-1">{t('results.cumulativeLayoutShiftDesc')}</div>
                          </div>
                        </div>
                      </motion.div>
                    )}
                    ...
                    {/* Issues Found */}
                    {(auditResult.rawData.issues || auditResult.comprehensiveResults?.issues) && (auditResult.rawData.issues || auditResult.comprehensiveResults?.issues)?.length > 0 && (
                      <motion.div
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                      ...
                      >
                        <h3 className="text-2xl font-semibold text-foreground mb-6 flex items-center">
                          ...
                          <div className="ml-auto bg-red-500/10 text-red-400 px-3 py-1 rounded-full text-sm font-medium">
                            {(auditResult.rawData.issues || auditResult.comprehensiveResults?.issues)?.length} {t('results.issues')}
                          </div>
                        </h3>

                        <div className="space-y-4">
                          {(auditResult.rawData.issues || auditResult.comprehensiveResults?.issues)?.map((issue: any, index: number) => (
                            <motion.div
                              key={index}
                                      ...
                                      {issue.recommendation && (
                                        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                                          <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <span className="text-sm font-medium text-blue-700 dark:text-blue-300">{t('results.howToFix')}</span>
                                          </div>
                                      ...
                                      <div className="flex items-center gap-4 text-sm mt-3">
                                        {issue.severity && (
                                          <div className="flex items-center gap-1">
                                            <span className="text-muted-foreground">{t('results.severity')}</span>
                    ...
                    {/* Quick Wins */}
                    {(auditResult.rawData.quick_wins || auditResult.comprehensiveResults?.quick_wins) && (auditResult.rawData.quick_wins || auditResult.comprehensiveResults?.quick_wins)?.length > 0 && (
                      <motion.div
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                      ...
                      >
                        <h3 className="text-2xl font-semibold text-foreground mb-6 flex items-center">
                          ...
                          <div className="ml-auto bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 px-3 py-1 rounded-full text-sm font-medium">
                            {(auditResult.rawData.quick_wins || auditResult.comprehensiveResults?.quick_wins)?.length} {t('results.improvements')}
                          </div>
                        </h3>

                        <div className="space-y-4">
                          {(auditResult.rawData.quick_wins || auditResult.comprehensiveResults?.quick_wins)?.map((win: any, index: number) => (
                            <motion.div
                              key={index}
                              initial={{ opacity: 0, x: -20 }}
                              animate={{ opacity: 1, x: 0 }}
