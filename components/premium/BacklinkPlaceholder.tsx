      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-600 mt-1 max-w-xl">
            Connect a premium data provider to unlock live backlink discovery, toxicity analysis, lost link alerts, and competitor gap insights. Below is a static demo of the JSON structure we surface.
          </p>
        </div>
        <div className="flex gap-2">
          <button className="px-3 py-1.5 text-sm rounded-md bg-black text-white hover:bg-slate-800" data-upsell="backlinks-upgrade">Upgrade</button>
          <button className="px-3 py-1.5 text-sm rounded-md border hover:bg-slate-100" onClick={()=>setExpanded(x=>!x)}>{expanded ? 'Hide JSON' : 'View JSON'}</button>
        </div>
      </div>
      {expanded && (
        <pre className="mt-4 max-h-72 overflow-auto text-xs bg-slate-900 text-slate-100 p-4 rounded"><code>{JSON.stringify(sampleData, null, 2)}</code></pre>
      )}
