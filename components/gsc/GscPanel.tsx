    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-3 text-sm mb-3">
        <div className="border rounded p-2">
          ...
          <div className="font-semibold">{insights.clicks ?? "—"}</div>
        </div>
        <div className="border rounded p-2">
          <div className="text-gray-600">Impressions</div>
          <div className="font-semibold">{insights.impressions ?? "—"}</div>
        </div>
        <div className="border rounded p-2">
          <div className="text-gray-600">CTR</div>
          <div className="font-semibold">
            {insights.ctr != null ? `${(insights.ctr * 100).toFixed(1)}%` : "—"}
          </div>
        </div>
      </div>
      <div>
        ...
        <ul className="text-sm list-disc pl-5 space-y-1">
          {insights.top_queries.slice(0, 10).map((q, i) => (
            <li key={i}>
              <span className="font-medium">{q.query}</span> — {q.clicks} clicks, {q.impressions}{" "}
              impressions, CTR {(q.ctr * 100).toFixed(1)}%, pos {q.position.toFixed(1)}
            </li>
          ))}
        </ul>
      </div>
