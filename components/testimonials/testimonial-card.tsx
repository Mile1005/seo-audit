...
export function TestimonialCard({ testimonial, isActive = false }: TestimonialCardProps) {
  ...
  const { name, title, company, companyLogo, avatar, rating, quoteKey, metric } = testimonial;
    ...
    >
      ...
      <div className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 border border-purple-500/20 rounded-xl p-4 mb-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="text-sm text-gray-400">{t(metric.descriptionKey)}</div>
          </div>
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="text-right">
          ...
          <div className="text-xs text-gray-500 mt-1 max-w-[80px] truncate">{company}</div>
        </div>
      </div>
    </motion.div>
  );
