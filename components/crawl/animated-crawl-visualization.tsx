...
export function AnimatedCrawlVisualization({
  url,
  maxPages = 25,
}: AnimatedCrawlVisualizationProps) {
            ...
            setTimeout(() => {
              const checks = [
                { type: "success" as const, msg: t("checks.metaValid") },
                {
                  ...
                  msg: t("checks.missingAlt", { count: Math.floor(Math.random() * 5) + 1 }),
                },
                { type: "success" as const, msg: t("checks.h1Present") },
                {
                  type: "warning" as const,
                  msg: t("checks.longTitle", { chars: 60 + Math.floor(Math.random() * 20) }),
                },
                {
                  type: "success" as const,
                  msg: t("checks.internalLinks", { count: Math.floor(Math.random() * 20) + 5 }),
                },
              ];
        ...
        <div className="relative z-10">
          ...
          <div className="grid grid-cols-4 gap-px bg-white/5">
            {[
              {
                icon: FileText,
                label: t("statsGrid.pages"),
                value: progress.pagesScanned,
                color: "blue",
              },
              {
                icon: Link,
                label: t("statsGrid.links"),
                value: Math.floor(progress.pagesScanned * 8.5),
                color: "purple",
              },
              {
                icon: ImageIcon,
                label: t("statsGrid.images"),
                value: Math.floor(progress.pagesScanned * 4.2),
                color: "cyan",
              },
              {
                icon: Code,
                label: t("statsGrid.issues"),
                value: Math.floor(progress.pagesScanned * 1.3),
                color: "orange",
              },
            ].map((stat, i) => (
              <motion.div
          ...
          <div className="px-4 pb-4">
            <div className="flex items-center gap-2 p-3 rounded-xl bg-gradient-to-r from-blue-500/10 to-purple-500/10 border border-blue-500/20">
              ...
              <span className="text-sm text-gray-300 truncate">
                {t("current.label")}:{" "}
                <span className="text-blue-300 font-mono">{progress.currentPage}</span>
              </span>
            </div>
          </div>
        </div>
