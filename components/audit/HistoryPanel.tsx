...
export function HistoryPanel({ currentUrl }: { currentUrl?: string }) {
  ...
  const [diff, setDiff] = useState<
    { newIssues: number; resolvedIssues: number; unchanged: number } | undefined
  >();
    ...
    const run = async () => {
      ...
      try {
        ...
        if (currentIssues.length || prevIssues.length) {
          ...
          setDiff({
            newIssues: d.newIssues.length,
            resolvedIssues: d.resolvedIssues.length,
      ...
      <CardContent className="space-y-6">
        ...
        {items.length > 0 && (
          <div className="p-4 bg-white dark:bg-slate-800 rounded-xl border">
            ...
            <div className="flex items-end justify-center">{sparkline(items)}</div>
          </div>
        )}
                    ...
                    <div className="space-y-1">
                      ...
                      {i.scores && (
                        <div className="flex gap-2 flex-wrap">
                          ...
                          <span className="text-xs text-slate-500 dark:text-slate-400">
                            Perf: {i.scores.performance}
                          </span>
                          <span className="text-xs text-slate-500 dark:text-slate-400">
                            A11y: {i.scores.accessibility}
                          </span>
                        </div>
                      )}
                    </div>
          ...
          <div className="p-6 bg-gradient-to-br from-green-50 via-blue-50 to-purple-50 dark:from-green-900/10 dark:via-blue-900/10 dark:to-purple-900/10 border-2 border-green-300 dark:border-green-700 rounded-xl">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-green-600 dark:text-green-400">
                    Analyzing improvements and changes
                  </p>
                </div>
              </div>
              <button
            ...
            {loadingDiff && (
              <div className="flex items-center gap-3 p-4 bg-blue-100 dark:bg-blue-900/20 rounded-lg">
                <div className="animate-spin w-5 h-5 border-2 border-blue-500 border-t-transparent rounded-full"></div>
                <span className="text-sm text-blue-700 dark:text-blue-300">
                  Analyzing differences between audit runs...
                </span>
              </div>
            )}
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-4 bg-white dark:bg-slate-800 rounded-lg border-l-4 border-blue-500">
                    ...
                    <div className="text-xs text-slate-600 dark:text-slate-400 mb-1">
                      ID: {selectedCurrent.slice(0, 8)} •{" "}
                      {new Date(
                        items.find((i) => i.id === selectedCurrent)?.createdAt || ""
                      ).toLocaleDateString()}
                    </div>
                  ...
                  <div className="p-4 bg-white dark:bg-slate-800 rounded-lg border-l-4 border-purple-500">
                    ...
                    <div className="text-xs text-slate-600 dark:text-slate-400 mb-1">
                      ID: {selectedBaseline.slice(0, 8)} •{" "}
                      {new Date(
                        items.find((i) => i.id === selectedBaseline)?.createdAt || ""
                      ).toLocaleDateString()}
                    </div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="text-center p-4 bg-gradient-to-br from-green-100 to-green-50 dark:from-green-900/20 dark:to-green-900/10 border border-green-200 dark:border-green-700 rounded-lg">
                    ...
                    <div className="text-sm font-medium text-green-600 dark:text-green-500 mb-1">
                      Issues Fixed
                    </div>
                    <div className="text-xs text-green-500 dark:text-green-600">
                      🎉 Great progress!
                    </div>
                  </div>
                  <div className="text-center p-4 bg-gradient-to-br from-red-100 to-red-50 dark:from-red-900/20 dark:to-red-900/10 border border-red-200 dark:border-red-700 rounded-lg">
                    ...
                    <div className="text-sm font-medium text-red-600 dark:text-red-500 mb-1">
                      New Issues
                    </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                    ...
                    <div className="text-center">
                      <div className="text-lg font-bold text-teal-600 dark:text-teal-400">
                        {Math.round(
                          ((diff.resolvedIssues - diff.newIssues) /
                            Math.max(1, diff.newIssues + diff.resolvedIssues + diff.unchanged)) *
                            100
                        )}
                        %
                      </div>
                      <div className="text-teal-700 dark:text-teal-300 text-xs">
                        Improvement Rate
                      </div>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-lg">
                    <h6 className="font-medium text-blue-800 dark:text-blue-200 mb-1 text-sm">
                      🎯 Next Steps
                    </h6>
                  ...
                  <div className="p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 rounded-lg">
                    <h6 className="font-medium text-green-800 dark:text-green-200 mb-1 text-sm">
                      📊 Monitor Progress
                    </h6>
            ...
            {!diff && !loadingDiff && (
              <div className="p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded-lg">
                <p className="text-sm text-yellow-800 dark:text-yellow-200">
                  💡 Unable to compare these audits. Both runs must be completed successfully and
                  contain detailed results for comparison.
                </p>
              </div>
            )}
          </div>
