      ...
      <div className="min-h-screen bg-slate-950">
        ...
        <section className="bg-slate-900/50 py-6">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <Breadcrumbs
              items={[{ name: "System Status", url: "https://www.aiseoturbo.com/status" }]}
              includeHome={true}
              darkMode={true}
            />
          </div>
        ...
        <section className="py-20 bg-slate-900">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
              ...
              <p className="text-gray-300 mb-6">{t("reliability.uptime.description")}</p>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {(
                  t.raw("reliability.uptime.features") as { title: string; description: string }[]
                ).map((feature, index) => (
            ...
            >
              <div className="flex items-center gap-4 mb-6">
                ...
                <h3 className="text-2xl font-bold text-white">
                  {t("reliability.transparency.title")}
                </h3>
              </div>
              <p className="text-gray-300 mb-6">{t("reliability.transparency.description")}</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {(
                  t.raw("reliability.transparency.methods") as {
                    title: string;
                    description: string;
                  }[]
                ).map((method, index) => (
            ...
            >
              <div className="flex items-center gap-4 mb-6">
                ...
                <h3 className="text-2xl font-bold text-white">
                  {t("reliability.maintenance.title")}
                </h3>
              </div>
              <p className="text-gray-300 mb-6">{t("reliability.maintenance.description")}</p>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {(
                  t.raw("reliability.maintenance.philosophy") as {
                    title: string;
                    description: string;
                  }[]
                ).map((item, index) => (
            ...
            >
              <h3 className="text-2xl font-bold text-white mb-6">
                {t("reliability.seoImpact.title")}
              </h3>
              <p className="text-gray-300 mb-6">{t("reliability.seoImpact.description")}</p>
              <div className="bg-green-900/30 border border-green-500/30 p-6 rounded-lg">
                <h4 className="text-green-200 font-semibold mb-3">
                  {t("reliability.seoImpact.testimonials.title")}
                </h4>
                <div className="space-y-4">
                  {(
                    t.raw("reliability.seoImpact.testimonials.items") as {
                      quote: string;
                      author: string;
                    }[]
            ...
            >
              <h3 className="text-2xl font-bold text-white mb-6">
                {t("reliability.enterprise.title")}
              </h3>
              <p className="text-gray-300 mb-6">{t("reliability.enterprise.description")}</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {(
                  t.raw("reliability.enterprise.features") as {
                    title: string;
                    description: string;
                  }[]
                ).map((feature, index) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        ...
                        <p className="text-sm text-gray-500">
                          {incident.date} • {t("incidents.durationLabel")}: {incident.duration}
                        </p>
                      </div>
                    </div>
        ...
        <section className="py-20 bg-slate-900/50">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            >
              ...
              <h2 className="text-3xl font-bold text-white mb-4">{t("subscribe.title")}</h2>
              <p className="text-xl text-gray-300 mb-8">{t("subscribe.description")}</p>
              <p className="text-gray-400">{t("subscribe.followText")}</p>
            </motion.div>
          </div>
        </section>
      </div>
