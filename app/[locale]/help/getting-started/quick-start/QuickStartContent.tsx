      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              {t.raw('progressTracker.steps').map((step: any, index: number) => (
                <div key={index} className="flex items-center gap-3 p-3 bg-slate-900/50 rounded-lg">
                  <div className="w-8 h-8 rounded-full bg-green-600 flex items-center justify-center text-white text-sm font-bold">{step.number}</div>
                  <div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="bg-slate-900/50 border border-slate-600 rounded-lg p-4 mb-4">
                    <h4 className="text-white font-medium mb-3">{t('steps.1.requirements.title')}</h4>
                ...
                <div className="flex-1">
                  ...
                  <div className="space-y-4">
                      ...
                      <div className="space-y-3">
                        {t.raw('steps.2.details.items').map((item: any, index: number) => (
                          <div key={index} className="flex items-center gap-3">
                            {item.icon === 'Globe' && <Globe className="w-5 h-5 text-blue-400" aria-hidden="true" />}
                            {item.icon === 'Settings' && <Settings className="w-5 h-5 text-purple-400" aria-hidden="true" />}
                            {item.icon === 'BarChart3' && <BarChart3 className="w-5 h-5 text-orange-400" aria-hidden="true" />}
                            <div>
                              <span className="text-gray-300 text-sm">{item.label}</span>
                              {item.value && <code className="ml-2 text-green-400 bg-slate-800 px-2 py-1 rounded text-sm">{item.value}</code>}
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="bg-yellow-900/30 border border-yellow-600/30 rounded-lg p-4">
                      <h4 className="text-yellow-400 font-medium mb-2">{t('steps.2.warning.title')}</h4>
                      <p className="text-gray-300 text-sm">
                        {t('steps.2.warning.description')}
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-4">
                    <div className="bg-slate-900/50 border border-slate-600 rounded-lg p-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {t.raw('steps.3.analysis.categories').map((category: string[], categoryIndex: number) => (
                          <div key={categoryIndex} className="space-y-2">
                            {category.map((item: string, itemIndex: number) => {
                              const colors = ['green', 'blue', 'purple', 'orange', 'yellow', 'cyan'];
                              const color = colors[(categoryIndex * 3 + itemIndex) % colors.length];
                              return (
            ...
            <div className="bg-slate-800/30 border border-slate-700 rounded-xl p-6">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="space-y-4">
                    <div className="bg-slate-900/50 border border-slate-600 rounded-lg p-4">
                      <h4 className="text-white font-medium mb-3">{t('steps.4.dashboard.title')}</h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {t.raw('steps.4.dashboard.items').map((item: any, index: number) => (
                          <div key={index} className="space-y-3">
                            <div className="flex items-start gap-3">
                              {item.icon === 'BarChart3' && <BarChart3 className="w-5 h-5 text-blue-400 mt-0.5" aria-hidden="true" />}
                              {item.icon === 'AlertTriangle' && <AlertTriangle className="w-5 h-5 text-yellow-400 mt-0.5" aria-hidden="true" />}
                              {item.icon === 'CheckCircle' && <CheckCircle className="w-5 h-5 text-green-400 mt-0.5" aria-hidden="true" />}
                              {item.icon === 'Target' && <Target className="w-5 h-5 text-purple-400 mt-0.5" aria-hidden="true" />}
                              <div>
                    ...
                    <div className="bg-purple-900/30 border border-purple-600/30 rounded-lg p-4">
                      <h4 className="text-purple-400 font-medium mb-2">{t('steps.4.focus.title')}</h4>
                      <p className="text-gray-300 text-sm">
                        {t('steps.4.focus.description')}
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-gradient-to-r from-blue-900/50 to-purple-900/50 border border-blue-700/50 rounded-xl p-8 mt-12">
            <h2 className="text-2xl font-bold text-white mb-6 text-center">{t('congratulations.title')}</h2>
              ...
              {t.raw('congratulations.nextActions').map((action: any, index: number) => (
                <div key={index} className="bg-slate-800/50 border border-slate-600 rounded-lg p-4 text-center">
                  <div className="w-12 h-12 rounded-lg bg-gradient-to-r from-green-500 to-emerald-500 p-3 mx-auto mb-3">
                    {action.icon === 'CheckCircle' && <CheckCircle className="w-6 h-6 text-white" aria-hidden="true" />}
                    {action.icon === 'BarChart3' && <BarChart3 className="w-6 h-6 text-white" aria-hidden="true" />}
                    {action.icon === 'Rocket' && <Rocket className="w-6 h-6 text-white" aria-hidden="true" />}
                  </div>
