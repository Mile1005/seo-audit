  ...
  try {
    ...
    if (!title) {
      ...
      issues.push({
        ...
        description: "Critical: Add a title tag to your page for SEO.",
        severity: "high",
      });
      ...
      checks.push({
        ...
        passed: false,
        severity: "medium",
        message: `Too short (${title.length} chars)`,
        value: title.length,
      });
      ...
      checks.push({
        ...
        passed: false,
        severity: "low",
        message: `Too long (${title.length} chars)`,
        value: title.length,
      });
    ...
    if (!metaDesc) {
      ...
      issues.push({
        ...
        description: "Add a meta description (120-160 chars) for better CTR.",
        severity: "high",
      });
    ...
    if (h1Tags.length === 0) {
      ...
      issues.push({
        ...
        description: "Add a primary H1 heading to define your page topic.",
        severity: "high",
      });
      ...
      checks.push({
        ...
        passed: false,
        severity: "medium",
        message: `${h1Tags.length} H1s found`,
        value: h1Tags.length,
      });
    ...
    if (!hasHierarchy && h2Count === 0)
      quickWins.push("Add H2 subheadings to structure your content");
    ...
    checks.push({
      ...
      severity: "high",
      message: url.startsWith("https://") ? "Secure" : "Not secure",
    });
    if (!url.startsWith("https://"))
      issues.push({
        title: "Not Using HTTPS",
        description: "Switch to HTTPS for security and SEO.",
        severity: "high",
      });
    ...
    checks.push({
      ...
      name: "Robots Meta",
      passed: !hasNoIndex,
      severity: hasNoIndex ? "high" : "low",
      message: robotsMeta || "Default (index)",
    });
    if (hasNoIndex)
      issues.push({
        title: "Page Set to NoIndex",
        description: "Remove noindex directive to allow indexing.",
        severity: "high",
      });
    ...
    checks.push({
      ...
      severity: "high",
      message: hasMobileViewport ? "Mobile-friendly" : "Missing",
    });
    if (!hasMobileViewport)
      issues.push({
        title: "Missing Mobile Viewport",
        description: "Add viewport meta for mobile rendering.",
        severity: "high",
      });
    ...
    checks.push({
      ...
      passed: favicon,
      severity: "low",
      message: favicon ? "Present" : "Missing",
    });
    ...
    checks.push({
      ...
      passed: jsonLd > 0,
      severity: "low",
      message: jsonLd > 0 ? `${jsonLd} blocks` : "None found",
      value: jsonLd,
    });
    ...
    checks.push({
      ...
      passed: !!csp,
      severity: "medium",
      message: csp ? "Configured" : "Not set",
    });
    ...
    checks.push({
      ...
      passed: !!hsts,
      severity: "low",
      message: hsts ? "Enabled" : "Not enabled",
    });
    ...
    const altPercent =
      images.length > 0
        ? Math.round(((images.length - imagesWithoutAlt) / images.length) * 100)
        : 100;
    checks.push({
      id: "img_alt",
    ...
    if (inputs.length > 0) {
      ...
      checks.push({
        ...
        category: "Accessibility",
        name: "Form Labels",
        passed: labeledInputs === inputs.length,
        severity: "medium",
        message: `${labelPercent}% labeled`,
        value: labelPercent,
      });
      if (labeledInputs < inputs.length)
        issues.push({
          title: "Unlabeled Form Inputs",
          description: `${inputs.length - labeledInputs} inputs need labels or aria-label.`,
          severity: "medium",
        });
    }
    ...
    if (buttons.length > 0) {
      ...
      checks.push({
        ...
        category: "Accessibility",
        name: "Button Labels",
        passed: labeledButtons === buttons.length,
        severity: "medium",
        message: `${btnPercent}% labeled`,
        value: btnPercent,
      });
    }

    const emptyLinks = $("a").filter(
      (_, el) => !$(el).text().trim() && !$(el).attr("aria-label")
    ).length;
    checks.push({
      id: "link_text",
      ...
      passed: emptyLinks === 0,
      severity: "medium",
      message: emptyLinks > 0 ? `${emptyLinks} empty links` : "All links labeled",
      value: emptyLinks,
    });
    ...
    checks.push({
      ...
      passed: skipLink,
      severity: "low",
      message: skipLink ? "Present" : "Missing",
    });
    ...
    if (loadTime > 1000)
      issues.push({
        ...
        description: `Server took ${loadSec}s. Aim for under 1s.`,
        severity: loadTime > 2000 ? "high" : "medium",
      });
    ...
    checks.push({
      ...
      name: "HTML Size",
      passed: sizeKB < 100,
      severity: sizeKB > 150 ? "medium" : "low",
      message: `${sizeKB}KB`,
      value: sizeKB,
    });
    ...
    const imgOptPercent =
      images.length > 0
        ? Math.round(((images.length - nonOptimizedImages) / images.length) * 100)
        : 100;
    checks.push({
      id: "img_format",
      category: "Performance",
      name: "Image Format",
      passed: imgOptPercent > 70,
      severity: "medium",
      message: `${imgOptPercent}% optimized`,
      value: imgOptPercent,
    });
    ...
    checks.push({
      ...
      passed: scripts < 10,
      severity: scripts > 15 ? "medium" : "low",
    ...
    checks.push({
      ...
      category: "Performance",
      name: "Async Scripts",
      passed: deferPercent > 60,
      severity: "medium",
      message: `${deferPercent}% async`,
      value: deferPercent,
    });
    ...
    const byCategory = checks.reduce(
      (acc, check) => {
