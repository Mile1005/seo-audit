...
export async function POST(req: NextRequest) {
  try {
    ...
    const existing = await (prisma as any).recurringAudit.findFirst({
      where: { url: normalized, frequency, isActive: true },
    });
    if (existing) {
    ...
    const created = await (prisma as any).recurringAudit.create({
      data: { url: normalized, frequency, nextRunAt },
    });
...
export async function GET() {
  try {
    const items = await (prisma as any).recurringAudit.findMany({
