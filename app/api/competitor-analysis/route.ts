...
function generateSerpComparison(keywords: string[]) {
  return keywords.map(keyword => ({
    ...
    competitors: [
      { name: 'semrush.com', rank: Math.floor(Math.random() * 5) + 1, change: Math.floor(Math.random() * 6) - 3 },
      { name: 'ahrefs.com', rank: Math.floor(Math.random() * 5) + 1, change: Math.floor(Math.random() * 6) - 3 },
      { name: 'screaming-frog.co.uk', rank: Math.floor(Math.random() * 5) + 1, change: Math.floor(Math.random() * 6) - 3 },
      { name: 'sitechecker.pro', rank: Math.floor(Math.random() * 5) + 1, change: Math.floor(Math.random() * 6) - 3 }
    ],
...
function generateGapAnalysis(targetUrl: string, competitorUrls: string[]) {
  return {
    keywordGaps: [
      { keyword: 'free seo audit tool', competitorRank: 3, yourRank: null, volume: 18000, difficulty: 45, opportunity: 'high' },
      { keyword: 'website analyzer online', competitorRank: 5, yourRank: null, volume: 12100, difficulty: 52, opportunity: 'medium' },
      { keyword: 'technical seo checker', competitorRank: 2, yourRank: 8, volume: 8900, difficulty: 38, opportunity: 'high' },
      { keyword: 'site audit software', competitorRank: 4, yourRank: null, volume: 15600, difficulty: 61, opportunity: 'medium' }
    ],
    contentGaps: [
