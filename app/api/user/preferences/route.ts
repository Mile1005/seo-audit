...
const memoryStore = new Map<string, { company: string; timezone: string; preferredLocale?: string }>()
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({
      preferences: prefs ? JSON.parse(prefs) : memoryStore.get(key) || { company: '', timezone: 'UTC', preferredLocale: 'en' }
    })
  } catch (e) {
