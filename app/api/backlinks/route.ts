...
export async function GET(request: NextRequest) {
  try {
    ...
    const [statusStats, linkTypeStats, domainRatingStats, toxicCount, referringDomainsCount] = await Promise.all([
    ...
    return NextResponse.json({
      ...
      stats: {
        ...
        byLinkType: linkTypeStats.reduce((acc, item) => {
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to process backlinks' },
      { status: 500 }
    )
  }
}
