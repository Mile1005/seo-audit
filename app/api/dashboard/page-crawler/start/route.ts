    ...
    (async () => {
      ...
      try {
        ...
        while (queue.length && pages.length < maxPages) {
          ...
          console.log(
            `[dashboard-crawl] ${id} processing ${pages.length + 1}/${maxPages}: ${current} (depth: ${depth})`
          );
          ...
          try {
            ...
            if (!rootComprehensiveAttached && current === url) {
              try {
                ...
                console.warn(
                  "[dashboard-crawl] comprehensive root audit failed",
                  (err as any)?.message
                );
              }
            }
        ...
        console.log(
          `[dashboard-crawl] ${id} completed: ${pages.length} pages crawled (max: ${maxPages}), queue remaining: ${queue.length}`
        );
        updateCrawl(id, (job) => {
          ...
          try {
            ...
            if (job) {
              ...
              const summary = {
                ...
                pagesWithIssues: job.pages.filter(
                  (p) =>
                    !p.title ||
                    p.h1Count === 0 ||
                    !p.metaDescription ||
                    (p.imagesWithoutAlt || 0) > 0
                ).length,
                averageWordCount:
                  job.pages.reduce((sum, p) => sum + (p.wordCount || 0), 0) / job.pages.length,
