...
export async function GET(req: NextRequest, { params }: { params: { domainId: string } }) {
  try {
    ...
    console.log(`Fetching rank snapshots for domainId: ${domainId}`);
    ...
    console.log(`Found ${snapshots.length} rank snapshots for domainId: ${domainId}`);
    return NextResponse.json(snapshots);
  } catch (error: any) {
    console.error(`Error fetching rank snapshots for domainId ${params.domainId}:`, error);
